...
var toc = (function () {
  ...
  var $_6r54zsbjkbnbab0 = { create: create };

  var tocId = $_6r54zsbjkbnbab0.create('mcetoc_');
  ...
  var readHeaders = function (editor) {
    var tocClass = $_f62sgvsajkbnbaay.getTocClass(editor);
    var headerTag = $_f62sgvsajkbnbaay.getTocHeader(editor);
    var selector = generateSelector($_f62sgvsajkbnbaay.getTocDepth(editor));
  ...
  var generateTocHtml = function (editor) {
    ...
    return '<div class="' + editor.dom.encode($_f62sgvsajkbnbaay.getTocClass(editor)) + '" contenteditable="false">' + html + '</div>';
  };
  var generateTocContentHtml = function (editor) {
    ...
    html += generateTitle($_f62sgvsajkbnbaay.getTocHeader(editor), global$2.translate('Table of Contents'));
    for (i = 0; i < headers.length; i++) {
      h = headers[i];
      h.element.id = h.id;
  ...
  var register = function (editor) {
    editor.addCommand('mceInsertToc', function () {
      $_avqg5qs6jkbnbaaq.insertToc(editor);
    });
    editor.addCommand('mceUpdateToc', function () {
      $_avqg5qs6jkbnbaaq.updateToc(editor);
    });
  };
  var $_etdukws5jkbnbaak = { register: register };

  var setup = function (editor) {
    var $ = editor.$, tocClass = $_f62sgvsajkbnbaay.getTocClass(editor);
  ...
  var $_dglgg2scjkbnbab3 = { setup: setup };

  var toggleState = function (editor) {
    return function (e) {
      var ctrl = e.control;
      editor.on('LoadContent SetContent change', function () {
        ctrl.disabled(editor.readonly || !$_avqg5qs6jkbnbaaq.hasHeaders(editor));
      });
    };
  };
  var isToc = function (editor) {
    return function (elm) {
      return elm && editor.dom.is(elm, '.' + $_f62sgvsajkbnbaay.getTocClass(editor)) && editor.getBody().contains(elm);
    };
  };
  ...
  var $_e5660bsdjkbnbab7 = { register: register$1 };

  global.add('toc', function (editor) {
    $_etdukws5jkbnbaak.register(editor);
    $_e5660bsdjkbnbab7.register(editor);
    $_dglgg2scjkbnbab3.setup(editor);
  });
  function Plugin () {
  }
